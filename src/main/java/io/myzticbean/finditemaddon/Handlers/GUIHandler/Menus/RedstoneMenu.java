...
public class RedstoneMenu extends CategoryPaginated {

    ArrayList<ItemStack> redstoneItems = new ArrayList<>();

    public RedstoneMenu(PlayerMenuUtility playerMenuUtility) {
        super(playerMenuUtility);
        addMenuPotions();
    }

    public void addMenuPotions() {
        redstoneItems.add(new ItemStack(Material.ACTIVATOR_RAIL));
        redstoneItems.add(new ItemStack(Material.COMPARATOR));
        redstoneItems.add(new ItemStack(Material.DETECTOR_RAIL));
        redstoneItems.add(new ItemStack(Material.DISPENSER));
        redstoneItems.add(new ItemStack(Material.DROPPER));
        redstoneItems.add(new ItemStack(Material.HOPPER));
        redstoneItems.add(new ItemStack(Material.OBSERVER));
        redstoneItems.add(new ItemStack(Material.PISTON));
        redstoneItems.add(new ItemStack(Material.POWERED_RAIL));
        redstoneItems.add(new ItemStack(Material.RAIL));
        redstoneItems.add(new ItemStack(Material.REPEATER));
        redstoneItems.add(new ItemStack(Material.STICKY_PISTON));
        redstoneItems.add(new ItemStack(Material.TRIPWIRE_HOOK));
    }

    @Override
    public String getMenuName() {
        return Utils.chat("&8Redstone Items");
    }
            ...
            if (player.hasPermission(PlayerPerms.FINDITEM_USE.value())) {
                ...
                if (FindItemAddOn.getQsApiInstance().isQSShopCacheImplemented()) {
                    Bukkit.getScheduler().runTaskAsynchronously(FindItemAddOn.getInstance(), () -> {
                         ...
                         */
                        FoundShopsMenu menu = new FoundShopsMenu(FindItemAddOn.getPlayerMenuUtility(player), searchResultList, ShopCategory.REDSTONE_ITEMS, mat, isBuying, SearchType.NORMAL);
                        Bukkit.getScheduler().runTask(FindItemAddOn.getInstance(), () -> menu.open(searchResultList));
                    });
                }
    ...
    private void handleLastPageClick(InventoryClickEvent event) {
        int listSize = redstoneItems.size();
        if (!((index + 1) >= listSize)) {
    ...
    public void setMenuItems() {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(FindItemAddOn.getInstance(), () -> {
            ...
            while (guiSlotCounter < super.maxItemsPerPage) {
                ...
                if (index >= redstoneItems.size()) break;

                if (redstoneItems.get(index) != null) {
                    // Place Search Results here
                    ItemStack itemStack = redstoneItems.get(index);
